...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="w-80 h-full flex flex-col overflow-hidden bg-graph-sidebar border-r border-graph-border">
      {/* Header */}
      <div className="p-4 border-b border-graph-border">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-graph-primary to-[#2d70b3] flex items-center justify-center">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-medium text-graph-muted uppercase tracking-wider">Equations</h2>
            <button
              onClick={onAddEquation}
              className="w-7 h-7 flex items-center justify-center rounded bg-graph-primary/10 text-graph-primary hover:bg-graph-primary/20 transition-colors"
              title="Add equation"
            >
        ...
        {variables.length > 0 && (
          <div className="p-4 border-t border-graph-border">
            <h2 className="text-sm font-medium text-graph-muted uppercase tracking-wider mb-3">Variables</h2>
        ...
        {detectedPoints.length > 0 && (
          <div className="p-4 border-t border-graph-border">
            <PointsPanel points={detectedPoints} equations={equations} onHighlightPoint={onHighlightPoint} />
          </div>
        )}
      </div>
